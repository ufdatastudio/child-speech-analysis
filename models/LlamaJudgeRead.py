...
4. Completeness / coverage of the child's reading content
...
# Data Loading
# ---------------------------------------------------------------------
def load_reading_csv(csv_path: str) -> List[Dict[str, str]]:
...
def evaluate_summaries(csv_path: str,
                       ...
                       reference_column: str = "LlamaRead_summary",
                       output_dir: str = "results/Reading/Evaluations",
                       tokenizer=None,
                       model=None,
                       max_new_tokens: int = 512,
                       temperature: float = 0.2):
    """Evaluate each model summary column against the reference summary."""
    rows = load_reading_csv(csv_path)
    ...
    for model_col in model_columns:
        if model_col not in rows[0]:
            print(f" Column '{model_col}' not found in CSV — skipping.")
            continue
        ...
        for i, row in enumerate(rows):
            ...
            try:
                ...
                if not scores:
                    print(f"   Retry due to malformed JSON ...")
                    response = generate_evaluation(reference, model_summary, tokenizer, model,
                                                   max_new_tokens=max_new_tokens, temperature=temperature)
                    scores = extract_json_from_response(response)
...
def main():
    ...
    parser.add_argument("--csv_path", type=str, required=True, help="Path to reading CSV file.")
    parser.add_argument("--model_columns", nargs="+", default=["AF35_summary", "Qwen_summary", "Kimi_summary", "Salmon_summary", "GAMA_summary"],
                        help="Model summary columns to evaluate.")
    parser.add_argument("--reference_column", type=str, default="LlamaRead_summary", help="Reference summary column.")
    parser.add_argument("--output_dir", type=str, default="results/Reading/Evaluations", help="Directory to save outputs.")
