...
#SBATCH --job-name mistral-judge-reading
#SBATCH --nodes=1
#SBATCH --gpus=2
...
CSV_PATH="${BASE_DIR}/results/reading/reading_combined.enriched.csv"
OUTPUT_DIR="${BASE_DIR}/results/Reading/MistralJudge"
...
echo "===== Starting Mistral Judge Evaluation (Reading) ====="
echo "CSV Path: ${CSV_PATH}"
echo "Output Dir: ${OUTPUT_DIR}"

# Run Mistral judge evaluation for reading
python3 -c "
import sys
sys.path.append('${BASE_DIR}')
from models.multi_open_judge_eval import *
...
for jname, jid in JUDGE_MODELS.items():
    try:
        evaluate_with_judge(
            ...
            judge_name=jname,
            judge_id=jid,
            model_columns=['AF35_summary', 'Qwen_summary', 'Kimi_summary', 'Salmon_summary', 'GAMA_summary'],
            reference_column='LlamaRead_summary',
...
print('Mistral reading evaluation complete')
"
